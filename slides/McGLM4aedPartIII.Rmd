...
## **Part II: Multivariate analysis of non-Gaussian data **
  ...
  - Goal 1: Make GLM more flexible to deal with:
    ...
    probability mass at zero (Bonat and Kokonendji, 2017).
    - Under, equi, overdispersed, zero-inflated and heavy-tailed 
    count data (Bonat et.al. 2017).
    - Bounded data (Bonat et. al. 2017).
  - Goal 2: Extend GLM to cGLM to deal with:
    - Mixed models (Pinheiro and Bates, 2000). 
    - Repeated measures (Diggle et. al. 1994).
    - Time series (Box and Jenkins, 1976).
    - Spatial and space-time data (Cressie, 1993; Knorr-Held, 2000).
    - Genetic and Twin data (Bonat, 2017; Wang et. al. 2011).
...
\mathrm{Var}(\boldsymbol{Y}) &=& \boldsymbol{\Sigma} = \mathrm{V}(\boldsymbol{\mu};p)^{\frac{1}{2}} ( \tau_0 \boldsymbol{I} ) \mathrm{V}(\boldsymbol{\mu};p)^{\frac{1}{2}}.
\end{eqnarray}
...
$$ \vartheta(\boldsymbol{\mu};p) = \mu^p$$ 
  ...
  - Poisson-Tweedie variance function 
  
  $$ \vartheta(\boldsymbol{\mu};p) = \mu + \mu^p \exp \left \{(2-p)\Phi_p(\mu)\right \}.$$
...
respectively. (See Bonat, 2017 for details.)
...
where $\mathrm{S}_{\boldsymbol{\theta}}^{-T} = (\mathrm{S}_{\boldsymbol{\theta}}^{-1})^{T}$.
 ...
 - For details, see Bonat and Jorgensen (2016) and Jorgensen and Knudsen (2004). 
...
where $\mathrm{J}_{\boldsymbol{\beta}} = \boldsymbol{D}^{\top} \boldsymbol{C}^{-1}\boldsymbol{D}.$
  ...
  - The Wald test corresponds to the Hotelling-Lawley statistics.
...
$\texttt{sqrt}$ and $\texttt{inverse}$.
  - Variance functions: $\texttt{constant}$, $\texttt{tweedie}$, $\texttt{poisson_tweedie}$, 
